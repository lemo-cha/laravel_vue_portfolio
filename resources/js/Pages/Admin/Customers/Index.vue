            ...
            <div class="l-page__container">
                <div class="p-content">
                    ...
                    <div class="p-content__text">
                        新規登録
                        <PrimaryButton @click="createCustomer">新規登録</PrimaryButton>
                    </div>
                </div>
            </div>
            <div class="l-page__container">
                <div class="p-content">
                    <div class="p-content__text">
                        <table class="p-content__table">
                            ...
                            <tr v-for="(customer,index) in customers.data" :key="customer.id" class="p-content__table--column"
                                @click="showCustomer(customer.id)">
                                    <td class="p-content__table--column-data">{{ index + 1 }}</td>
                                    <td class="p-content__table--column-data">{{ customer.custom_id }}</td>
                                    <td class="p-content__table--column-data">{{ customer.name }}</td>
                                    <td class="p-content__table--column-data">{{ customer.is_active }}</td>
                            </tr>
                        </table>
                        <Pagination :links="customers.links" class="p-paginate" />
                    </div>
                </div>
            </div>
        </div>
